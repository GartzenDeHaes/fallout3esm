...
namespace TES3Unity
{
	...
	public class BSAFile : IDisposable
	{
		...
		private void ReadMetadata()
		{
			...
			uint hashTableOffsetFromEndOfHeader = _reader.ReadUInt32(); // minus header size (12 bytes)
			...
			for (int i = 0; i < fileCount; i++)
			{
				FileMetadatas[i].size = _reader.ReadUInt32();
				FileMetadatas[i].offsetInDataSection = _reader.ReadUInt32();
			}
			...
			for (int i = 0; i < fileCount; i++)
			{
				filenameOffsets[i] = _reader.ReadUInt32();
			}
			...
			for (int i = 0; i < fileCount; i++)
			{
				FileMetadatas[i].pathHash.value1 = _reader.ReadUInt32();
				FileMetadatas[i].pathHash.value2 = _reader.ReadUInt32();
			}
